...
{
    ESP_LOGI(TAG, "initialize(): begin");

    // initialize configuration arguments
    ESP_LOGI(TAG, "initialize(): configuring arguments");
    if (init_config_args() != ESP_OK)
        return false;

    // initialize GPIO
    ESP_LOGI(TAG, "initialize(): configuring GPIO");
    if (init_gpio() != ESP_OK)
        return false;

    // initialize HINT ISR
    ESP_LOGI(TAG, "initialize(): configuring HINT ISR");
    if (init_hint_isr() != ESP_OK)
        return false;

    // initialize SPI
    ESP_LOGI(TAG, "initialize(): configuring SPI bus");
    if (init_spi() != ESP_OK)
        return false;

    // initialize SH2 HAL
    ESP_LOGI(TAG, "initialize(): configuring SH2 HAL");
    if (init_sh2_HAL() != ESP_OK)
        return false;

    // initialize tasks
    ESP_LOGI(TAG, "initialize(): launching tasks");
...
{
    ESP_LOGI(TAG, "init_sh2_HAL(): begin");
    ...
    // reset BNO08x
    ESP_LOGI(TAG, "init_sh2_HAL(): toggling reset");
    toggle_reset();

    ESP_LOGI(TAG, "init_sh2_HAL(): calling sh2_open()");
    ...
    ESP_LOGI(TAG, "init_sh2_HAL(): sh2_open() returned");

    memset(&product_IDs, 0, sizeof(sh2_ProductIds_t));

    ESP_LOGI(TAG, "init_sh2_HAL(): fetching product IDs");
    ...
    ESP_LOGI(TAG, "init_sh2_HAL(): product IDs received");
...
void BNO08x::toggle_reset()
{
    ...
    // Give the IMU time to boot before the first host transaction asks it for data.
    vTaskDelay(POST_RESET_SETTLE_MS);
}
